...
def get_data():
    """
    Fetch and return the CDC Diabetes Health Indicators dataset.
    ...
    return X, y, cdc_diabetes_health_indicators.metadata
...
def main():
    """
    Main function to demonstrate data preprocessing and transformation functions.
    """
    print("="*80)
    print("DATA PREPROCESSING AND TRANSFORMATION EXPERIMENTS")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 1: Basic Preprocessing")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 2: Zero Mean Normalization")
    print("="*80)
    X_zero_mean, means = zero_mean_normalize(X_basic)
    print(f"Mean values after normalization (should be ~0):\n{X_zero_mean.select_dtypes(include=[np.number]).mean().head()}")
    
    print("\n" + "="*80)
    print("EXPERIMENT 3: Standard Scaling")
    print("="*80)
    X_scaled, scaler = standardize_features(X_basic, method='standard')
    print(f"Mean: {X_scaled.select_dtypes(include=[np.number]).mean().abs().max():.6f}")
    print(f"Std: {X_scaled.select_dtypes(include=[np.number]).std().abs().max():.6f}")
    
    print("\n" + "="*80)
    print("EXPERIMENT 4: Feature Selection - Variance Threshold")
    print("="*80)
    X_var_selected, selected_features, _ = feature_selection_variance(X_basic, threshold=0.01)
    
    print("\n" + "="*80)
    print("EXPERIMENT 5: Feature Selection - Univariate (Top 15)")
    print("="*80)
    X_uni_selected, selected_features, _ = feature_selection_univariate(X_basic, y_basic, k=15)
    print(f"Selected features: {selected_features}")
    
    print("\n" + "="*80)
    print("EXPERIMENT 6: Feature Selection - Model-Based")
    print("="*80)
    X_model_selected, selected_features, _ = feature_selection_model_based(X_basic, y_basic, max_features=20)
    print(f"Selected features: {selected_features}")
    
    print("\n" + "="*80)
    print("EXPERIMENT 7: Feature Selection - Correlation-Based")
    print("="*80)
    X_corr_selected, selected_features, removed = feature_selection_correlation(X_basic, threshold=0.95)
    print(f"Removed {len(removed)} highly correlated features")
    
    print("\n" + "="*80)
    print("EXPERIMENT 8: Advanced Pipeline with Scaling + Feature Selection")
    print("="*80)
    X_advanced, transformers = preprocessing_pipeline_advanced(
        X_basic, y_basic,
        scaling='standard',
    ...
    print("\n" + "="*80)
    print("EXPERIMENT 9: PCA Transformation")
    print("="*80)
    X_pca, pca_obj, explained_var = apply_pca(X_basic, variance_ratio=0.95)
    print(f"Top 5 components explain: {explained_var[:5]}")
    
    print("\n" + "="*80)
    print("EXPERIMENT 10: Robust Scaling (for outlier-resistant preprocessing)")
    print("="*80)
    X_robust, robust_scaler = standardize_features(X_basic, method='robust')
    ...
    print(f"  Basic preprocessing: {X_basic.shape}")
    print(f"  Zero mean normalized: {X_zero_mean.shape}")
    print(f"  Standard scaled: {X_scaled.shape}")
    print(f"  Variance selected: {X_var_selected.shape}")
    print(f"  Univariate selected: {X_uni_selected.shape}")
    print(f"  Model-based selected: {X_model_selected.shape}")
    print(f"  Correlation filtered: {X_corr_selected.shape}")
    print(f"  Advanced pipeline: {X_advanced.shape}")
    print(f"  PCA transformed: {X_pca.shape}")
    print(f"  Robust scaled: {X_robust.shape}")
